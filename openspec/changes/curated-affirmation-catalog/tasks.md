...
- [x] 4.1 RED `ResolveSelectedGroupIdsTest` (D18, extend existing Android-free suite): `persisted = null` -> all 14 universes + `personalizadas`; `persisted = {"personalizadas","bienestar"}` w/ new 15 `knownIds` -> falls back to the 14 (**the Pro-tier bug case**); `persisted = {"personalizadas","self_worth"}` -> returned verbatim (healthy selection untouched); `persisted = emptySet()` -> fallback fires. Plus wiring assertion: `defaultAffirmationGroups().filter{isThematic && requiredTier==FREE}` has size 14. **This is the explicit RED-first Pro-tier invariant fix (D18) — write before touching `resolveSelectedGroupIds`.**
- [x] 4.2 GREEN `data/AffirmityAppState.kt`: `defaultThematicGroupIds` re-expressed via `isThematic`; `resolveSelectedGroupIds` gains the thematic-emptiness fallback, tier-independent (moves invariant out of the `AccessTier.FREE`-guarded `deselectLockedGroups` branch).
- [x] 4.3 RED legacy-removal test: `selectableAffirmationGroups()` has 15 entries, contains `personalizadas`, contains none of the 3 legacy ids; `resolveSelectedGroupIds(persisted=setOf("bienestar"),…)` drops it and lands on a valid default. New file `AffirmationGroupCatalogTest.kt`.
- [x] 4.4 GREEN `ui/groups/AffirmationGroup.kt`: delete `bienestar`/`autocuidado`/`fuerza_de_voluntad`; `defaultAffirmationGroups() = catalogUniverseGroups()` (14).
- [x] 4.5 GREEN `res/values/strings.xml`, `res/values-en/strings.xml`: delete the 6 legacy group strings; add 14 group titles + 14 descriptions per locale (from `universes[].title`/`coreNeed`). **Note (PR2 deviation, now closed):** the 14 `values/strings.xml` (es) title/description pairs were added early, in PR2; this task deleted the 6 legacy strings from both locales and added the missing 14 English title/description pairs to `values-en/strings.xml` (translated from the Spanish originals), per the design's own Affected-Areas table (`design.md:1000`), which explicitly calls for both locales — the "Spanish only" spec note (spec.md:254) refers to locale/i18n *infrastructure* (no `locale` field on data), not to skipping the existing `values-en/` UI-string convention the app already ships.
- [x] 4.6 GREEN — **fixture repair, explicit tasks, not discovered mid-apply.** Repoint the 4 suites that source Free/Pro/PER_USE group fixtures from `defaultAffirmationGroups()` by literal legacy id to locally-constructed `AffirmationGroup` fixtures:
  - [x] 4.6.1 `GroupAccessPolicyTest` — 3 local `AffirmationGroup` fixtures (Free/Pro/ProOrAdPerUse shapes).
  - [x] 4.6.2 `AdUnlockEndToEndTest` — 1 local `AffirmationGroup` fixture (Pro-or-ad-PER_USE shape), id kept literal to match the suite's existing `knownGroupIds`/`proOnlyGroupIds` sets.
  - [x] 4.6.3 `AffirmityAppStateAdFunnelAnalyticsTest` — only `.id` was used; replaced with a literal `fuerzaDeVoluntadId` string, no `AffirmationGroup` construction needed.
  - [x] 4.6.4 `AccessDecisionPurityAnalyticsTest` — **verified, no change needed**: this suite calls `defaultAffirmationGroups()` generically (iterates whatever it returns) and never references a legacy id literally; it compiles and passes unchanged against the new 14-universe list.
  - **Deviation, not in the original 4-suite list**: `CatalogIdSchemeTest` (`data/catalog/`, built in PR2/task 2.5) also broke — it asserted catalog groupIds never collide with `defaultAffirmationGroups().map(id) + personalizadas`, which became tautologically false once `defaultAffirmationGroups()` **is** `catalogUniverseGroups()` (the catalog's own universe ids). Fixed by comparing against a literal `{"bienestar","autocuidado","fuerza_de_voluntad"}` set plus `personalizadas`, preserving the real invariant (no catalog id collides with `personalizadas` or a deleted legacy id) without the self-referential tautology.
  - Acceptance: each suite resolves (compiles) and passes without referencing `bienestar`/`autocuidado`/`fuerza_de_voluntad` via `defaultAffirmationGroups()`. Suites using legacy ids as opaque strings (`ContentKeyTest`, `AccessResolutionTest`, `FirestorePathsTest`, `FirestoreMappersTest`, `AdUnlockDaoTest`, `AdUnlockMigrationTest`, `EntitlementResolutionTest`) are **not** touched — confirmed, `EntitlementResolutionTest` only gained one new regression test (4.7), no existing test lines changed.
- [x] 4.7 Regression check: `proOnlyGroupIds` (`AffirmityAppState.kt`) now evaluates empty in production (all 14 universe groups are `ContentAccess.Free`); added `EntitlementResolutionTest`'s "is a documented no-op once proOnlyIds is empty" test asserting `deselectLockedGroups` stays a provable no-op in that state — the function is documented, not deleted.

## Phase 5: PR5 — Selector UI, partial-lock badge, seeder script, acceptance

- [x] 5.1 RED `CatalogAccessPolicyTest` (D19): `deriveCatalogBadge` — unlocked+`isPartiallyLocked=true` -> `PARTIALLY_LOCKED`; unlocked+`false` -> null; **locked group -> PREMIUM/AD_UNLOCK, never PARTIALLY_LOCKED**; `personalizadas` -> its own PREMIUM override, never partial. Regression: full existing `GroupAccessPolicyTest` badge table passes byte-for-byte (`deriveBadge` untouched).
- [x] 5.2 RED `CatalogAccessPolicyTest`: `partiallyLockedGroupIds` — `PRO` -> `emptySet()`, zero collection resolutions; `FREE` + empty grants -> exactly `CATALOG_GATED_GROUP_IDS`; `FREE` + live grant covering the only gated collection -> that universe drops out; expired grant -> universe returns. Plus consistency assertion: `CATALOG_GATED_GROUP_IDS` equals the set derived from `catalogCollections()` at runtime.
- [x] 5.3 GREEN `ui/groups/CatalogAccessPolicy.kt`: `deriveCatalogBadge`, `partiallyLockedGroupIds`. `ui/groups/AffirmationGroup.kt`: add `GroupBadge.PARTIALLY_LOCKED`. `ui/groups/GroupAccessPolicy.kt` **left unmodified** — do not widen `deriveBadge`.
- [x] 5.4 GREEN `ui/groups/AffirmationGroupSelectorSheet.kt`: new `partiallyLockedIds: Set<String> = emptySet()` param, threaded to the row; badge call site uses `deriveCatalogBadge`; new `PARTIALLY_LOCKED` branch in `AffirmationGroupAccessBadge`. Lock/toggle/dim logic untouched. **Deviation, scope-narrowed**: the sticky-header section restructuring (*Mis afirmaciones*/*Temáticas*, moving Favorites/Add-custom cards above the group list) described in design.md's "Selector at 15 groups" section was explicitly recommended as presentation-only and independently sliceable, not specified as a hard requirement with concrete mockups — deferred as a follow-up UI polish item, not silently dropped. The list still scrolls correctly at 15 rows today (unchanged `LazyColumn` inside `fillMaxHeight(0.85f)`), so nothing is broken, only findability is unoptimized.
- [x] 5.5 GREEN `MainActivity.kt`: `partiallyLockedIds` memoized on `(entitlementTier, adUnlockState)`, passed to selector sheet.
- [x] 5.6 GREEN `res/values/strings.xml` (es "Incluye Pro"), `res/values-en/strings.xml` (en "Includes Pro"): `affirmation_group_badge_partial`.
- [x] 5.7 **Badge-uniformity acceptance check (blocking, pre-merge):** inspected the generator's emitted `CATALOG_GATED_GROUP_IDS` directly in the committed `CatalogTaxonomy.kt` — **measured 14/14 universes gated** (every one of the 14 universes has >=1 non-Free collection). Confirms design D19's uniformity caveat is real, not hypothetical: the badge is uniform across all 14 rows on day one. Recorded here per the acceptance task; the cheap follow-up ("12 de 18 gratis" count) remains explicitly out of scope per design.
- [x] 5.8 GREEN `functions/tools/seedCatalog.ts` (new): Admin SDK, chunk at <=450 ops/batch, taxonomy first then affirmations, `catalogMeta/version` written last, idempotent `set(merge:true)`. **Deviation**: `--catalog` must point at the FULL source JSON (`{catalogVersion, universes, themes, collections, affirmations}`, same shape `generate-catalog.mjs` consumes), not the trimmed bundled `app/src/main/assets/catalog.v1.json` asset the design's literal CLI example named — the bundled asset carries no taxonomy/access data (measured: it is `{version, affirmations}` only), so it cannot populate `catalogUniverses`/`catalogThemes`/`catalogCollections`. Documented in the script's header comment.
- [x] 5.9 GREEN `functions/test/seedCatalog.test.ts` (13 tests, all passing): chunk boundaries respected; taxonomy committed before any affirmation write; marker is the last op of the last commit (its own single-write batch); a second run is a no-op-equivalent (identical write sequence); a mid-run abort (including on the last content chunk) leaves no marker committed. **Deviation**: exercised against a fake `BatchCommitter`, not the real Firestore emulator — `firebase` CLI is not installed in this sandbox (same environment blocker as `test/firestore.rules.test.ts`'s emulator suites, tasks 1.17/3.14). `seedCatalog()`'s chunking/ordering/marker-last contract lives entirely in its own code, independent of which committer executes it, so this is full coverage of the contract, not a placeholder — only the real network round-trip against Firestore is unexercised.
- [x] 5.10 **Gap found in PR3 review, unassigned until now:** `CatalogSeeder` (built in PR2) is never invoked anywhere in the app-startup path — the local `catalog_affirmations` Room table stays empty on a real device without this. RED: a startup-path test asserting `CatalogSeeder.seedIfNeeded()` is invoked once, idempotently, on `AffirmityAppState` construction. GREEN: wired. `AffirmityAppState` gained an optional `catalogSeeder: CatalogSeeder? = null` constructor param (`null` for every existing JVM test, so none needed touching); `init` launches `scope.launch(Dispatchers.IO) { catalogSeeder?.seedIfNeeded() }` as its first statement — off the main thread (D2's cold-start requirement), safe on every launch (idempotent via `CatalogPreferences.seededCatalogVersion`, D13). `rememberAffirmityAppState()` wires the real `CatalogSeeder(AndroidCatalogAssetReader(context), database.catalogAffirmationDao(), AndroidCatalogPreferences(context))`. Two new tests in `AffirmityAppStateCatalogTest.kt` assert exactly-once invocation and idempotent no-op on an already-seeded marker.
- [ ] 5.11 Manual/on-device acceptance (blocking, before merge, not provable by unit suite) — **NONE of the items below were executed in this sandbox: no physical device or emulator is available.** Left unchecked deliberately rather than fabricated. Must be run on a real device before this change ships.
  - [ ] 5.11.1 D8: catalog card with empty subtitle renders with no layout gap. **UNEXECUTED — needs a real device.**
  - [ ] 5.11.2 Cold-start seed of 2712 rows does not visibly block first paint. **UNEXECUTED — needs a real device.**
  - [ ] 5.11.3 15-row selector navigable, Aplicar reachable. **UNEXECUTED — needs a real device.**
  - [ ] 5.11.4 D19: partially-locked row shows badge, remains checkable, not dimmed. **UNEXECUTED — needs a real device.**
  - [ ] 5.11.5 D18: fresh install (cleared app data) opens with all 14 universes checked. **UNEXECUTED — needs a real device.**
  - [ ] 5.11.6 D16: watch ad on a `TIMED_REPEATABLE` collection -> unlocks; advance device clock past window -> re-locks and re-offers ad (not `LockedNeedsPro`); watch again -> second grant persists. **UNEXECUTED — needs a real device with ad inventory.**
  - [ ] 5.11.7 5.10's seeder wiring actually populates `catalog_affirmations` on a real first launch (confirms 5.11.2/5.11.3/5.11.5 have real data behind them, not an empty table). **UNEXECUTED — needs a real device.** The unit-level wiring (seeder is invoked exactly once, idempotently) IS covered by `AffirmityAppStateCatalogTest`'s two new tests (task 5.10), but that is not a substitute for confirming real rows land in a real on-device Room database.

## Phase 6: Cross-cutting close-out (part of PR5)

- [x] 6.1 Confirm `AffirmationsScreen` received no new parameter (D9) — verified: `git diff 9a01816..HEAD` shows zero changes to `ui/affirmations/AffirmationsScreen.kt` across the whole PR5 slice.
- [x] 6.2 Confirm `FavoriteAffirmationDao`/`FavoriteAffirmationEntity`/`FavoriteAffirmationRepository` unmodified (D10) — verified: zero diff on all three files across PR5.
- [x] 6.3 Confirm no `MIGRATION_9_8` was added and `fallbackToDestructiveMigrationOnDowngrade` stays disabled (D15) — verified: `rg` for both strings in `AffirmityDatabase.kt` finds no `MIGRATION_9_8` anywhere in the repo and no `fallbackToDestructiveMigrationOnDowngrade` call site at all (never enabled).
